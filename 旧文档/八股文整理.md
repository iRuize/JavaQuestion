...
​ 初始化桶的数量为 16，桶的最大数量为$2^{30}$,10 亿多,默认设定负载因子是 0.75，表示有数据的桶的数量占比75%的时候会进行扩容操作；余下几个参数和树化有关系。当一个桶内的键值对达到 8 条之后并且当前桶的数量已经到了 64（历经两次扩容，分别是有数据的桶数量有 12,24 的时候），此时桶内的数据才会由链表的形式转变为红黑树的形式。
