...
​ 初始化桶的数量为 16，桶的最大数量为$2^{30}$,10 亿多,默认设定负载因子是 0.75，表示有数据的桶的数量占比75%的时候会进行扩容操作；余下几个参数和树化有关系。当一个桶内的键值对达到 8 条之后并且当前桶的数量已经到了 64（历经两次扩容，扩容的时机分别是有数据的桶数量有 12,24 的时候），此时桶内的数据才会由链表的形式转变为红黑树的形式。
...
1. HashSet、HashMap、HashTable 等集合在存储元素时，底层依赖的是 哈希值 + equals 比较
第一步：调用对象的 hashCode() 计算哈希值，决定应该放在哪个桶（位置）；

第二步：如果哈希值相同，再调用 equals() 判断是否“真的是同一个对象”

新建了一个person对象，属性值只有一个名字，分别是p1和p2，豆角John，重写equals方法，但是没有重写hashcode方法，那么当我们将这两个对象放入hashMap中时，第一步根据hashcode判断，两个对象内存地址不同，存入不同的桶，这样就是两个“人”了，这样跟我们初始期望是不一样的
#### Java 多线程问题
